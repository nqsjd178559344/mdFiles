...
6. 约定式提交 规范 https://www.conventionalcommits.org/zh-hans/v1.0.0/
...
5. gap
   ...
   1. column-gap / row-gap

6. display: flex 与 display:-webkit-box 的区别

   1. 超出一行显示省略号的 css 中, 后三者配套使用
      ```
        overflow: hidden;
...
2.  typescript 类型判定
    ...
    2.  转为某规则可能过于保守，并且不允许可能有效的更复杂的强制转换。如果发生这种情况，您可以使用两个断言，首先是 any（或 unknown，我们稍后会介绍），然后是所需的类型：
        const a = (expr as unknown) as T;
    3.  as const
    4.  类型谓词 https://www.kelen.cc/posts/612da7024971412461d8e3ad
    5.  ```tsx

        ```

    6.  type ToArray<Type> = Type extends any ? Type[] : never => ToArray<string|number> = ToArray<string> | ToArray<number> = string[] | number[]
    7.  type ToArray<Type> = [Type] extends [any] ? Type[] : never =>ToArray<string|number> = (string | number)[]
...
1. 点击按钮时， 获取 Column 的 slider 的 当前值
...
https://mp.weixin.qq.com/s/pVI1pmZdNIRWguoV5HKAeg
