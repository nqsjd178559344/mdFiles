...
4. 关注 组件库[如 tailwindcss 等]的新版本，及了解并使用新功能
...
1.  转为某规则可能过于保守，并且不允许可能有效的更复杂的强制转换。如果发生这种情况，您可以使用两个断言，首先是 any（或 unknown，我们稍后会介绍），然后是所需的类型：
    const a = (expr as unknown) as T;
1.  as const
1.  类型谓词 https://www.kelen.cc/posts/612da7024971412461d8e3ad
1.  ```tsx

    ```

1.  type ToArray<Type> = Type extends any ? Type[] : never => ToArray<string|number> = ToArray<string> | ToArray<number> = string[] | number[]
1.  type ToArray<Type> = [Type] extends [any] ? Type[] : never =>ToArray<string|number> = (string | number)[]
