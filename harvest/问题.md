...
21. [常用css布局](https://www.ruanyifeng.com/blog/2020/08/five-css-layouts-in-one-line.html)
...
### echarts
1. 暂不支持 tooltip.rich => 用formatter替代
2. tooltip超出边界 
   ```tsx
    const gap = 15
   ...
   ```
3. [legend.textStyle.width失效](https://github.com/apache/echarts/issues?q=legend.width)，会影响到翻页按钮，使其同样不生效 =>  
   ```tsx
   ...
   ```
4. Sankey图.emphasis.focus = 'adjacency'不生效 =>
   ```tsx
