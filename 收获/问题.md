...
6. html2canvas 对 css 部分属性(z-index, 伪元素等)缺少支持
7. [pnpm patch-package](https://juejin.cn/post/7119369833187115039)
8. 组件库的打包需求：组件分别提供 esm、commonjs、umd 三种模块规范的代码，并且还有单独打包出的 css。
9. 前端性能优化
...
1. 将修改过的 csv 文档 重置回 csv 文档
   ...
   3. 重新保存设置
2. Monorepo: 简单来说，就是将原本应该放到多个仓库的代码放入一个仓库，让它们共享相同的版本控制。这样可以降低代码复用成本。
